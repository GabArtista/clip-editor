...
Este documento descreve os componentes planejados para evoluir o editor automático de vídeos em uma plataforma SaaS com backend escalável.
...
- **Autenticação (JWT):** emissão de tokens Bearer para acesso aos demais módulos e isolamento de dados por usuário.
- **Apps clientes (futuros):** integrações leves (web/mobile) podem consumir a API atual conforme as necessidades de produto evoluírem.

## Fluxo Alto Nível

1. Usuário autentica em um cliente (web ou mobile futuro) e vincula sua conta do Instagram. Cookies são criptografados e persistidos.
...
4. **Interfaces cliente:** evoluir protótipos web/mobile somente quando necessário, reaproveitando a API existente.
5. **CI/CD:** configurar pipelines (GitHub Actions/GitLab CI) para rodar testes e lint do backend.
