...
from app.infrastructure.database.models.video_edit_model import VideoEditModel
...
class VideoEditRepository(IVideoEditRepository):
    ...
    def get_by_id(self, video_edit_id: int) -> Optional[VideoEdit]:
        ...
        db_video = self.db.query(VideoEditModel).filter(
            VideoEditModel.id == video_edit_id
        ).first()
    ...
    def get_by_user_id(self, user_id: int, skip: int = 0, limit: int = 100) -> List[VideoEdit]:
        ...
        db_videos = self.db.query(VideoEditModel).filter(
            VideoEditModel.user_id == user_id
        ).offset(skip).limit(limit).order_by(VideoEditModel.created_at.desc()).all()
    ...
    def get_pending_approval(self, user_id: int) -> List[VideoEdit]:
        ...
        db_videos = self.db.query(VideoEditModel).filter(
            and_(
                VideoEditModel.user_id == user_id,
                VideoEditModel.status == VideoEditStatus.PENDING_APPROVAL
            )
        ).order_by(VideoEditModel.created_at.desc()).all()
        return [v.to_domain() for v in db_videos]
    ...
    def get_expired_previews(self, current_time: datetime) -> List[VideoEdit]:
        ...
        db_videos = self.db.query(VideoEditModel).filter(
            and_(
                VideoEditModel.status == VideoEditStatus.PENDING_APPROVAL,
                VideoEditModel.expires_at < current_time
            )
        ).all()
        return [v.to_domain() for v in db_videos]
    ...
    def get_to_delete(self, current_time: datetime) -> List[VideoEdit]:
        ...
        db_videos = self.db.query(VideoEditModel).filter(
            and_(
                VideoEditModel.delete_at.isnot(None),
                VideoEditModel.delete_at < current_time
            )
        ).all()
        return [v.to_domain() for v in db_videos]
    ...
    def update(self, video_edit: VideoEdit) -> VideoEdit:
        ...
        db_video = self.db.query(VideoEditModel).filter(
            VideoEditModel.id == video_edit.id
        ).first()
        if not db_video:
